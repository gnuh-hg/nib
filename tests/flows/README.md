...
> Thư mục này chứa **kế hoạch test** dạng `.md` cho app Nib ("notepad toán học sống").
...
>
> **Cấu trúc tests/ (ISSUE-25 Phương án A — tách theo vai trò):**
> ```
> tests/
> ├── flows/        # Kế hoạch: .flow.md + README + _TEMPLATE (thư mục này)
> ├── e2e/          # Playwright spec .ts — 1 flow = 1 spec, cùng slug
> └── evidence/     # Screenshot/GIF/console — commit làm baseline regression
> test-results/     # Playwright temp output — gitignore (KHÔNG commit)
> playwright-report/ # Playwright HTML report — gitignore (KHÔNG commit)
> ```
> Config: `playwright.config.ts` ở root (testDir=`tests/e2e`, baseURL :1420).
...
| **Playwright spec** | `tests/e2e/<feature-slug>.spec.ts` — 1 flow = 1 spec, **cùng slug** với `.flow.md`. Chạy: `npx playwright test tests/e2e/<slug>.spec.ts`. Config: `playwright.config.ts` ở root (testDir=`tests/e2e`, baseURL :1420). |
| **Evidence** | Screenshot/GIF/console-log lưu trong `tests/evidence/<slug>/` — **có commit** (baseline regression). Flow file trỏ tới evidence + ghi PASS/FAIL. |
...
| [free-caret-v2-phase-a.flow.md](free-caret-v2-phase-a.flow.md) | Gate vàng Phase A free-caret-v2: spacer-atom + virtual-caret + materialize-on-input + **multi-segment §3b** (6 lớp tương đương: trống/trái/giữa-text/phải/giữa-2-đoạn/combo) | executed | 2026-06-30 |
