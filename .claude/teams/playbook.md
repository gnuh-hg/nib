# playbook — orchestration thao tác (note-ch)

> "Bộ não" thao tác của **lead** khi vận hành note-ch như native Claude Code team (Agent Teams).
> Đọc cùng `.claude/master.md` (3 bất biến + roster 8 vai + vòng lặp TaskList loop + subagent-vs-teammate).
...
2. Spawn teammate cần dùng:
   ...
   - **Sau khi spawn N teammate → áp layout terminal (§8). Guard = KIỂM `$TMUX` RUNTIME, KHÔNG đọc
     ...
     → **`$TMUX` set (tmux đang chạy)**: PHẢI áp lệnh layout §8 tương ứng N (main-vertical N=2..3, 2×2 grid
     N=4, 2+2+1 N=5) — KHÔNG được bỏ sót (defect ISSUE-2/6).
...
```
context:       <tóm tắt research/plan/thiết kế teammate cần; path tuyệt đối nếu dài, vd /home/gnuh/Documents/project/note-ch/plan/...>
...
## 5. Done-criteria implementer — gate idiom note-ch
...
Config: `"teammateMode": "in-process"` trong **project** `.claude/settings.json` (note-ch), hoặc flag `claude --teammate-mode in-process`.
...
### tmux pane layout cho N teammate (note-ch realistic, N=2..5)
...
tmux move-window -r -s 1:1 && tmux select-layout main-vertical     # Block 2 (sau kill 1 pane)
```
...
**N=5** — full chain note-ch (researcher → planner → architect → editor-frontend → backend-cas), main + 2+2+1:
```
┌──────────────┬──────────┬──────────┐
│              │ tm1 (p2) │ tm2 (p3) │
│  main (p1)   ├──────────┼──────────┤
...
**Teammate:**
9. **Silent-complete** — xong không SendMessage report → team treo (issue `SILENT`).
10. **Peer-DM teammate khác** — không qua lead trừ khi brief chỉ định (issue `SCOPE`).
11. **Làm ngoài scope brief** — tự lấy task khác từ TaskList / mở rộng phạm vi (issue `SCOPE`).
12. **Tự thoát không chờ shutdown_request** — mất kết quả cuối (issue `NO-SHUTDOWN-RESP`).
13. **Phán "có vẻ pass" / "render đẹp"** — gate phải từ exit-code/output thật (issue `GATE`).
14. **Trao đổi plan-as-data JSON giữa teammate** — giao tiếp prose markdown (issue `FORM`). (File code `src/`/`backend/` là artifact — KHÁC, hợp lệ.)
15. **Quên TaskUpdate(in_progress/completed)** — lead không track được trạng thái (issue `FORGOT-TASKUPDATE`).
16. **implementer đụng vùng vai khác** — `editor-frontend` sửa `backend/`, `team-ops` sửa `src/`... SAI vai. Mỗi vai giữ ranh giới file của mình (master §2).
...
- 3 bất biến + roster 8 vai + vòng lặp TaskList loop + subagent-vs-teammate: `.claude/master.md`.
