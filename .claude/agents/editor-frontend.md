...
Khi task yêu cầu **animation, transition, scroll-motion, micro-interaction** UI → **tự Read skill tương ứng TRƯỚC khi code**. GSAP là thư viện animation chính thức cho stack React+TS+Vite của note-ch. `npm install gsap` và `npm install @gsap/react` khi cần (đừng làm trước khi có task thật).
...
| **gsap-frameworks** | `.claude/skills/gsap-frameworks/SKILL.md` | Vue/Svelte (KHÔNG ưu tiên — stack note-ch là React; chỉ đọc nếu brief yêu cầu rõ). |
